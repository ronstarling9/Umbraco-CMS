...
public class UsersViewModelsMapDefinition : IMapDefinition
{
    ...
    {
        mapper.Define<PasswordChangedModel, ResetPasswordUserResponseModel>(
            (_, _) => new ResetPasswordUserResponseModel(), Map);
        mapper.Define<UserCreationResult, CreateUserResponseModel>(
            (_, _) => new CreateUserResponseModel { User = new() }, Map);
    ...
    private void Map(UserCreationResult source, CreateUserResponseModel target, MapperContext context)
    {
        Guid userKey = source.CreatedUser?.Key
                      ?? throw new ArgumentException(
                          "Cannot map a user creation response without a created user", nameof(source));
    ...
    // Umbraco.Code.MapAll
    private void Map(
        UserExternalLoginProviderModel source, UserExternalLoginProviderResponseModel target, MapperContext context)
    {
