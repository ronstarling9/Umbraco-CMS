...
public class TemporaryFileConfigurationPresentationFactory : ITemporaryFileConfigurationPresentationFactory
{
    ...
    [Obsolete(
        "Use the constructor that accepts IOptionsSnapshot<ContentImagingSettings> and does not accept" +
        " IImageUrlGenerator instead. Scheduled for removal in Umbraco 19.")]
    ...
    {
        // IImageUrlGenerator parameter is ignored - kept for DI compatibility with existing registrations,
        // but not used in the factory. This is to avoid breaking changes when adding the new constructor
        // that accepts IOptionsSnapshot<ContentImagingSettings>.
    }
